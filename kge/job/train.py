    ...
    def create(
        ...
        if config.get("train.type") == "KvsAll":
            return TrainingJobKvsAll(
                config, dataset, parent_job, initialize_for_forward_only,
            )
...
class TrainingJobKvsAll(TrainingJob):
    ...
    def __init__(
        self, config, dataset, parent_job=None, initialize_for_forward_only=False
    ):
        super().__init__(config, dataset, parent_job, initialize_for_forward_only)
